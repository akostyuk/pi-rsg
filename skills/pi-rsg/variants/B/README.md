...
This directory holds an **alternative execution mode** for pi-rsg in which
the main agent delegates each chapter to an isolated `chapter-investigator`
sub-agent via pi's subagent tool, instead of writing the chapter body
itself.
...
2. From Phase 3 onwards, the main agent reads
   ...
   chapter authoring via the subagent tool with `subagent_type =
   "chapter-investigator"` configured against
   `variants/B/chapter-investigator.md`.

Mode B is **not** required for normal pi-rsg use; treat this directory as
a reference variant.
