...
description: |
  Sub-agent that investigates a single pi-rsg chapter in an isolated
...
> body via the Write tool into `rds/analysis/<session_name>/drafts/NN-slug.md`; the return value
...
```
Chapter NN saved: rds/analysis/<session_name>/drafts/NN-slug.md (XXX lines, NN refs, N code blocks, N mermaid)
...
Manifest line to append (the main agent appends this to `rds/analysis/<session_name>/manifest.md`):
| NN | slug | rds/analysis/<session_name>/drafts/NN-slug.md | INV-xxx,INV-yyy | XXX lines | short key-topic phrase |
```
...
3. appends the manifest line to `rds/analysis/<session_name>/manifest.md`,
