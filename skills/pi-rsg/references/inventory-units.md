...
1. **First, run `python -m scripts.source_map_v2 --target <root>`**. This auto-extracts role-typed source units (SRC-NNNN) with tree-sitter-based language awareness and saves them to `rds/analysis/<session_name>/source-map.json`. Falls back to file-level units with a loud warning if tree-sitter is not installed.
